...
<head>
    ...
    <title>@yield('titulo', 'Cooperativa River Mall')</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('panel') }}" class="{{ request()->routeIs('panel') ? 'active' : '' }}"><i class="bi bi-grid-1x2-fill"></i><span>Panel principal</span></a>
        <div class="menu-title">Administración</div>
        <a href="{{ route('socios.index') }}" class="{{ request()->routeIs('socios.*') ? 'active' : '' }}"><i class="bi bi-people-fill"></i><span>Socios</span></a>
        <a href="{{ route('conductores.index') }}" class="{{ request()->routeIs('conductores.*') ? 'active' : '' }}"><i class="bi bi-person-vcard-fill"></i><span>Conductores</span></a>
        <a href="{{ route('taxis.index') }}" class="{{ request()->routeIs('taxis.*') ? 'active' : '' }}"><i class="bi bi-taxi-front-fill"></i><span>Taxis</span></a>
        <a href="{{ route('clientes.index') }}" class="{{ request()->routeIs('clientes.*') ? 'active' : '' }}"><i class="bi bi-person-lines-fill"></i><span>Clientes</span></a>
        <div class="menu-title">Operaciones</div>
        <a href="{{ route('servicios.index') }}" class="{{ request()->routeIs('servicios.*') ? 'active' : '' }}"><i class="bi bi-geo-alt-fill"></i><span>Servicios</span></a>
        <a href="{{ route('pagos.index') }}" class="{{ request()->routeIs('pagos.*') ? 'active' : '' }}"><i class="bi bi-cash-coin"></i><span>Pagos</span></a>
    </nav>
...
<div class="main">
    <header class="topbar">
        <div class="d-flex align-items-center gap-3">
            ...
            <div><h2>@yield('encabezado', 'Panel administrativo')</h2><p>SYSTAX TECNOSOLUCIONES FHL S.A.S.</p></div>
        </div>
        <div class="dropdown">
            <button class="user-menu" data-bs-toggle="dropdown" aria-expanded="false">
                <span class="user-avatar">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                <span class="d-none d-sm-block text-start"><strong>{{ auth()->user()->name }}</strong><small>Administrador</small></span>
                <i class="bi bi-chevron-down"></i>
            </button>
            <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                <li><span class="dropdown-item-text small text-muted">{{ auth()->user()->email }}</span></li>
                <li><hr class="dropdown-divider"></li>
                <li><form method="POST" action="{{ route('cerrar-sesion') }}">@csrf<button class="dropdown-item text-danger" type="submit"><i class="bi bi-box-arrow-right me-2"></i>Cerrar sesión</button></form></li>
            </ul>
        </div>
    </header>
    ...
    <footer class="app-footer"><span>© {{ date('Y') }} Cooperativa River Mall</span><span>Sangolquí, Quito · Ecuador</span></footer>
</div>
...
<div class="toast-container position-fixed top-0 end-0 p-3">
    <div id="mensajeToast" class="toast border-0 shadow-lg text-bg-{{ session('success') ? 'success' : 'danger' }}" role="alert">
        <div class="d-flex"><div class="toast-body"><i class="bi {{ session('success') ? 'bi-check-circle-fill' : 'bi-exclamation-triangle-fill' }} me-2"></i>{{ session('success') ?? session('error') }}</div><button class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast" aria-label="Cerrar"></button></div>
    </div>
</div>
@endif
